...
class IntraDynamic(Estimator):
    ...
    def __init__(
        ...
        self.layer_dynamic = layer_dynamic
    ...
    def __str__(self):
        return f"IntraDynamic_{self.layer_selection}_{self.embeddings_type}_{self.dynamic_name}_{self.layer_dynamic}"

    def _create_layer_model(self, layer: int, regression: bool):
    ...
    def _prepare_layer_stats(self, stats: Dict[str, np.ndarray], layer: int, train_idx=None, dev_idx=None):
        ...
        if train_idx is not None and dev_idx is not None:
            ...
            if f"train_layerwise_metrics_{self.dynamic_name}_{layer}" in stats.keys() and self.layer_dynamic:
                train_metrics_raw = stats[f"train_layerwise_metrics_{self.dynamic_name}_{layer}"]
                regression = True
            if self.final_dynamic:
                train_metrics = stats[f"train_layerwise_metrics_{self.dynamic_name}_agg"]
            ...
            return layer_stats, train_metrics[dev_idx], regression
        else:
            # Inference phase
            return stats
    ...
    def _train_layer_models(self, stats: Dict[str, np.ndarray], train_idx, dev_idx):
        ...
        for layer in self.layers:
            ...
            layer_stats, dev_labels, regression = self._prepare_layer_stats(stats, layer, train_idx, dev_idx)

            # Create layer model
            layer_model = self._create_layer_model(layer, regression)
